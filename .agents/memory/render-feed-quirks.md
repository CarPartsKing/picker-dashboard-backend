...
- ~~Gap detection sorts raw time strings~~ — **wrong diagnosis, corrected 2026-09-25.** The v3 and v4 Apps Scripts sort times as numbers. The real cause was 12-hour afternoon times read as early morning: 202 of 670 rows in the Sep 24 export started before 6 AM. Fixed in Apps Script v4 and in the dashboard's xlsx parser with the shift-hours rule. The shift runs 6 AM to about 8 PM. A time before 6:00 is PM. A time from 6:00 to 8:30 is PM once the day has reached noon, otherwise AM. A time from 8:31 to 11:59 is AM. The rule lives in `resolveTimeEntries_` in the .gs and `resolveShiftTimes` in parseUtils.ts; keep them in sync. Rows from old exports (May 3, Jun 30, Jul 23) keep their wrong times.
- Pickers type 12-hour colon times (e.g. "5:09" = 5:09 PM) with no AM/PM; see the shift-hours rule above. Some pickers' formats still fail (Brandon, Bryan never get L/Hr).
